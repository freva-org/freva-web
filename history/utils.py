...
def sendmail_to_follower(history_id, django_user, subject, message):
    '''
    sends a mail to all follower of the result, except the django user
    :type history_id: int
    :param history_id: the id of the history record
    :type django_user: django user object
    :param: django user as found in request
    :type subject: string
    ...
    followers = HistoryTag.objects.filter(history_id_id=history_id).filter(type=HistoryTag.tagType.follow)
    
    followers = follower.order_by('uid')
    ...
    text += reverse('history:result', kwargs={'history_id': history_id}) + '?action=unfollow'

    from_email = django_user.email
    to_email = []
