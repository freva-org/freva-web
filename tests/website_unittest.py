...
class EvaluationSystemWeb(unittest.TestCase):
    ...
    def start_tool(self, tool, caption=""):
        ...
        if caption == "":
            self.assertEqual(
                tool + " results - Evaluation System",
                self.driver.title,
                "Tool could not be started",
            )
        else:
            self.assertEqual(
                caption + " (" + tool.upper() + ") results - Evaluation System",
                self.driver.title,
                "Tool could not be started",
            )
    ...
    def test_data_browser(self):
        ...
        self.assertEqual(
            "Near-Surface Air Temperature", elem.get_attribute("data-original-title")
        )
    ...
    def test_tools(self):
        ...
        # test caption
        self.assertEqual(
            testcaption + " (" + tool.upper() + ")",
            self.driver.find_element_by_id("result_caption").text,
        ...
        self.assertEqual(
            tool + " setup - Evaluation System",
            self.driver.title,
            "Edit config not loaded",
        )
    ...
    def test_results(self):
        self.driver.get(
            "https://www-miklip.dkrz.de/history/" + self.id_to_test + "/results/"
        )
        self.assertEqual(
            "MOVIEPLOTTER results - Evaluation System",
            self.driver.title,
        ...
        self.assertEqual(
            self.driver.find_element_by_css_selector("div#status_div > p").text,
            "Sent to Sebastian Illing",
            "Mail not sent",
        )
        ...
        time.sleep(1)
        self.assertEqual(
            "test caption (MOVIEPLOTTER)", self.driver.title, "Cant set caption"
        )
        self.click_link("Set Caption")
        time.sleep(2)
